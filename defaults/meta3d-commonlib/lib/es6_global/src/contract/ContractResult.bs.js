...
function requireCheck(f, isTest) {
  ...
  try {
    return Result$Meta3dCommonlib.succeed(Curry._1(f, undefined));
  }
  catch (raw_e){
    var e = Caml_js_exceptions.internalToOCamlException(raw_e);
    if (e.RE_EXN_ID === Js_exn.$$Error) {
      return Result$Meta3dCommonlib.fail(e._1);
    }
    throw e;
  }
}
...
function ensureCheck(returnVal, f, isTest) {
  if (!isTest) {
    return Result$Meta3dCommonlib.succeed(returnVal);
  }
  try {
    return Result$Meta3dCommonlib.mapSuccess(Result$Meta3dCommonlib.succeed(Curry._1(f, returnVal)), (function (param) {
                  return returnVal;
                }));
  }
  catch (raw_e){
